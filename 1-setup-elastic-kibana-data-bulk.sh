...
echo "About to create the index: product-supplychain-consumer..."
echo "... add bulk add the data from an external file: product-supplychain-consumer.json ..."
echo "... where each row is known as a document in elasticsearch, and setting the document indexes as the numbers in the path"

curl -s -H "Content-Type: application/json" -XPOST http://localhost:9200/product-supplychain-consumer/docs/_bulk --data-binary "@1-product-supplychain-consumer.json"
